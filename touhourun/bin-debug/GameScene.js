var GameScene = (function (_super) {
    ...
    function GameScene() {
        ...
        this._marisa.x = 300;
        this._marisa.y = 335;
        var boxShape = new p2.Box({ width: 18, height: 24 });
        ...
        this._jumpMax = 80;
        this._jumpNum = 0;
        var bm = new egret.Bitmap(RES.getRes("walk_down1_png"));
        this.addChild(bm);
        // bm.x = 200;
        // bm.y = 200;
    ...
    p.__onSceneTap = function (evt) {
        if (this._marisa.state == 0) {
            this._marisa.state = 1;
        }
    };
    p.__onBmTap = function (evt) {
        this._marisa.x = 300;
    };
    p.__onMapInited = function (evt) {
        this.addEvent();
        egret.Ticker.getInstance().register(function (dt) {
            ...
            if (!this._isDebug) {
                ...
                if (this._marisaBody) {
                    var box = this._marisaBody.displays[0];
                    if (box) {
    ...
    p.checkHorizon = function () {
        var nx = this._marisa.x + this._marisa.width + this._speed;
    ...
    p.calHorizonTile = function (tile) {
    ...
    p.checkVertical = function () {
        var ny = this._marisa.y + this._marisa.height + this._gravity;
    ...
    p.calVerticalTile = function (tile) {
        var isblock = tile.getPropertyByName("isblock");
        if (isblock && isblock.value == "true") {
